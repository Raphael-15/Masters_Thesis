...
- Operational constraints: Charging and discharging power are constrained by the battery's power limits and instantaneous SoC (to avoid overcharge/overdischarge). The model can enforce non-simultaneous charge and discharge (via binary or complementarity approximations) or allow both with efficiency loss; the chosen implementation is documented in the Model_Formulation document.
...
- Cost elements: CAPEX and OPEX for PV and battery systems, replacement costs (battery repowering), and costs associated with degradation are included. Economic evaluation uses a discount rate and lifetime to compute NPV, payback, and LCOE/LCOS.

- Degradation: The model incorporates a throughput- or cycle-based degradation proxy (capacity fade as a function of cumulative energy throughput or equivalent full cycles). Degradation affects available capacity over time and informs replacement scheduling and lifecycle costs.
...
This curated, non-verbatim excerpt keeps the conceptual clarity needed to implement or review the model. If you want, I will now commit this file as docs/Model_Formulation_excerpt.md and also add a companion file with the key equations (LaTeX-style) and placeholders for figures/tables.